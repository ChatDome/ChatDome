...
`chatdome hello` 启动可编辑的终端会话，支持历史记录、slash 补全和 Agent 输出保护。非 TTY 输入使用普通 stdin/stdout。设置 `CHATDOME_PROMPT` 可替换或隐藏输入提示符。任务暂停时输入 `y` 继续，输入 `n` 停止。
...
| `/details [ID]` | 查看待审批动作详情 |
| `/confirm [ID]` | 批准待审批命令 |
| `/reject [ID]` | 拒绝待审批命令或停止暂停任务 |
| `/continue` | 继续暂停中的任务 |
| `/retry` | 重试上一次失败请求 |
