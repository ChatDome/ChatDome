...
`chatdome hello` 启动可编辑的终端会话，支持历史记录、slash 补全和 Agent 输出保护。非 TTY 输入使用普通 stdin/stdout。设置 `CHATDOME_PROMPT` 可替换或隐藏输入提示符。使用 `chatdome hello --quiet` 或 `CHATDOME_COMPACT=1` 可启用单行启动页。审批时输入 `y` 允许、`n` 拒绝、`d` 查看详情；任务暂停时输入 `y` 继续、`n` 停止。
