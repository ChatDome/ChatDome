...
from chatdome.runtime_paths import sentinel_alerts_path, sentinel_push_state_path
...
class SentinelScheduler:
    ...
    def __init__(
        ...
        self._history = AlertHistory(
            alerts_path=sentinel_alerts_path(),
            retention_days=config.alert_retention_days,
        )
        self._alert_push_state_path = sentinel_push_state_path()
    ...
    async def _run_sentinel_check(self, check_id: str, args: dict[str, Any] | None = None):
        with log_origin("sentinel"):
            return await self._sandbox.execute_sentinel_check(check_id, args=args)
    ...
    async def _ensure_auditd_status(self, force: bool = False) -> None:
        ...
        result = await self._run_sentinel_check("auditd_status", args={})
    ...
    async def _resolve_audit_session_id(self, sshd_pid: str) -> str:
        ...
        result = await self._run_sentinel_check(
    ...
    async def _discover_active_ssh_sessions(self, tracking_window_hours: int) -> list[SSHSessionRecord]:
        ...
        if self._pack_command_available("ssh_active_sessions"):
            result = await self._run_sentinel_check("ssh_active_sessions", args={"limit": 50})
