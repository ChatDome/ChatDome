...
LLM 必须返回当前子命令的安全状态、风险级别、修改和删除标记、影响说明以及命令组成信息。ChatDome 严格校验字段、枚举、布尔类型和风险一致性。首次响应无效时，在同一个 180 秒时间预算内使用精简契约重试一次；仍失败或超时则只标记当前子命令不可用，并继续分析下一条。

单条子命令的完整分析最多等待 180 秒。完整详情的截止时间为开始时间加 `180 × n` 秒，其中 `n` 是本地拆分后的子命令数量。分析严格串行，因此不会对同一模型并发发出详情请求。已经完成的结果不会因后续子命令失败而丢失。
